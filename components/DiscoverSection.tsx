        ...
        <div className="w-full">
          <div className="grid h-[360px] grid-cols-12 grid-rows-12 gap-2 sm:h-[440px] sm:gap-3 lg:h-[500px]">
            {collageImages.map((image, index) => (
              <ScrollReveal key={image.src} className={image.className} delay={index * 0.08} y={16}>
                <div className="group relative h-full overflow-hidden rounded-lg shadow-soft">
                  <Image
                    src={image.src}
                    alt={image.alt}
                    fill
                    className="object-cover transition duration-500 group-hover:scale-[1.035] group-active:scale-[1.035]"
